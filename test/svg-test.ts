...
describe("SVG", function () {
	...
	let svg: SVGMock
	let black: ISVGTypes.ColorStruct = { red: 0x0, green: 0x0, blue: 0x0, alpha: 0xFF }
	let one: ISVGTypes.ColorStruct = { red: 0x1, green: 0x1, blue: 0x1, alpha: 0xFF }
	let white: ISVGTypes.ColorStruct = { red: 0xFF, green: 0xFF, blue: 0xFF, alpha: 0xFF }
	let zeroOneTwo: ISVGTypes.ColorStruct = { red: 0x0, green: 0x1, blue: 0x2, alpha: 0xFF }
	let middleRandom: ISVGTypes.ColorStruct = { red: 126, green: 126, blue: 128, alpha: 0xFF } // rgb even, results in 50%
	let brightestRandom: ISVGTypes.ColorStruct = { red: 254, green: 254, blue: 254, alpha: 0xFF } // rgb even, results in 100%
	...
	it("Should brighten by 0%", async function () {
		expectColorToEqual(await svg.brightenColor(black, 0, 0), 0x000000, "black->black")
		expectColorToEqual(await svg.brightenColor(black, 0, 1), 0x010101, "black->one")
		expectColorToEqual(await svg.brightenColor(one, 0, 0), 0x010101, "one->one")
		expectColorToEqual(await svg.brightenColor(white, 0, 0), 0xFFFFFF, "max limit")
	})

	it("Should brighten by 3%", async function () {
		expectColorToEqual(await svg.brightenColor(black, 3, 0), 0x000000, "black->black")
		expectColorToEqual(await svg.brightenColor(black, 3, 1), 0x010101, "black->one")
	...
	var expectColorToEqual = (color: ISVGTypes.ColorStruct, expected: number, message: string = "") => {
